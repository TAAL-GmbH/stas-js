...
describe("atomic swap with 20 tokens", function () {
  // this swap function won't be used but is here as a sanity check
  it("Swap - All in one swap with 20 sat", async function () {
    ...
    expect(await utils.getVoutAmount(swapTxid, 0)).to.equal(0.0000002);
    expect(await utils.getVoutAmount(swapTxid, 1)).to.equal(0.0000002);
    await utils.isTokenBalance(aliceAddr, 20);
    await utils.isTokenBalance(bobAddr, 20);
  });

  it("Swap - 2 step token-p2pkh swap with 20 sat", async function () {
    const makerVout = 0;
    const takerVout = 0;
    ...
    expect(await utils.getVoutAmount(swapTxid, 1)).to.equal(0.0000002);
    await utils.isTokenBalanceTwoTokens(bobAddr, 40);
    await utils.isTokenBalance(aliceAddr, 0);
  });

  it("Swap - 2 step p2pkh-token swap with 20 sat", async function () {
    const takerStasInputScriptHex = tokenAObj.outputs[0].script.toHex();
    ...
    await utils.isTokenBalanceTwoTokens(aliceAddr, 40);
    await utils.isTokenBalance(bobAddr, 0);
  });

  it("Swap 3 step Token-Token with 20 sat", async function () {
    const takerStasInputScriptHex = tokenAObj.outputs[0].script.toHex();
    const makerStasInputScript = tokenBObj.outputs[0].script;
  ...
  it("Swap - 3 step token-p2pkh swap  with 20 token", async function () {
    // first get some funds
    const bobUtxos = await getFundsFromFaucet(
      bobPrivateKey.toAddress(process.env.NETWORK).toString()
    );
  ...
  it("Swap - 3 step p2pkh-token swap with 20 token", async function () {
    const takerStasInputScriptHex = tokenAObj.outputs[0].script.toHex();
    // first get some funds
    const aliceUtxos = await getFundsFromFaucet(
      alicePrivateKey.toAddress(process.env.NETWORK).toString()
