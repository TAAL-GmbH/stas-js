...
// needs fixed
describe("Swap With Fewer sats than in offered utxo", function () {
  ...
  it("Swap - 2 step p2pkh-token swap with fewer sats requested than offered UTXO", async function () {
    ...
    const wantedInfo = {
      ...
      satoshis: makerOutputSatoshis - 2000,
    };
  ...
  // the maker offers a token for sats
  it("Swap - 3 step token-p2pkh swap with fewer sats requested than offered UTXO", async function () {
    // first get some funds
    const bobUtxos = await getFundsFromFaucet(
      bobPrivateKey.toAddress(process.env.NETWORK).toString()
    );
    ...
    const takerInputInfo = {
      type: "native",
      utxo: bobUtxos[0],
      satoshis: takerInputSatoshis - 1000,
    };
    ...
    const takerSignedSwapHex = acceptUnsignedNativeSwapOffer(
  ...
  // the maker offers sats for a token
  it("Swap - 3 step p2pkh-token swap with fewer sats requested than offered UTXO", async function () {
    const takerStasInputScriptHex = tokenAObj.outputs[0].script.toHex();
    // first get some funds
    const aliceUtxos = await getFundsFromFaucet(
      alicePrivateKey.toAddress(process.env.NETWORK).toString()
    ...
    const wantedInfo = {
      scriptHex: takerStasInputScriptHex,
      satoshis: makerOutputSatoshis - 1000,
    };
    ...
    // now bob takes the offer
    const takerSignedSwapHex = acceptUnsignedSwapOffer(
      unsignedSwapOfferHex,
      makerInputTx,
      bobPrivateKey,
      tokenAIssueHex,
    ...
    const fullySignedSwapHex = makerSignSwapOffer(
      takerSignedSwapHex,
      makerInputTx,
      tokenAIssueHex,
      alicePrivateKey,
