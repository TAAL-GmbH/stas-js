...
function createSwapOffer (makerPrivateKey, makerInputUTXO, wantedInfo) {
  ...
  if (isMakerOutputStasScript) {
    ...
    const makerSignature = bsv.Transaction.sighash.sign(tx, makerPrivateKey, sighash, 0, makerInputUTXO.script, new bsv.crypto.BN(makerInputUTXO.satoshis), flags)
...
/*
    ...
    makerInputVout: the output index of the output the maker is offering
    -
    takerInputTxHex: the whole tx hex containing the output the taker is offering
    takerInputVout: the output index of the output the taker is offering
    -
...
function acceptSwapOffer (offerTxHex, makerInputTxHex,
  takerPrivateKey, takerInputTxHex, takerInputUTXO, takerOutputSatoshis, makerPublicKeyHash, paymentUtxo, paymentPrivateKey, makerPrivateKey) {
  ...
  const takerStasTokenTxid = bsv.Transaction(takerInputTx).hash
  ...
  const takerWantedScript = bsv.Script.fromHex(takerStasTokenrequest)
...
function acceptUnsignedSwapOffer (offerTxHex, makerInputTxHex,
  ...
  //   const makerInputSatoshis = makerInputTx.outputs[makerInputVout].satoshis
...
function acceptUnsignedNativeSwapOffer (offerTxHex, takerInputInfo, makerInputTxHex,
  ...
  //   console.log('makerOutputSatoshis2: ', makerOutputSatoshis)
...
function makerSignSwapOffer (offerTxHex, makerInputTxHex, takerInputTx, makerPrivateKey, takerPublicKeyHash, paymentPublicKeyHash, paymentUtxo) {
  ...
  console.log('makerInputVout', makerInputVout)
  console.log('takerInputVout', takerInputVout)
  ...
  //   console.log('makerInputTx[outpointIndex].script:', makerInputTx.outputs[outpointIndex].script.toHex())
...
function allInOneSwap (makerPrivateKey, makerInputUtxo, wantedInfo, makerInputTxHex, makerInputVout,
  ...
  const takerStasInputTx = bsv.Transaction(takerInputTxHex)
  const takerStasInputScript = takerStasInputTx.outputs[takerInputVout].script
  ...
  const reversedFundingTXID = reverseEndian(paymentUtxo.txid)
  console.log('makerUnlockScript: makerInputVout', makerInputVout)
  console.log('takerUnlockScript: takerInputVout', takerInputVout)
