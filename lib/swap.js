...
/*
    the taker signs for each of the rest of the transaction inputs (both funding and
    spending ones of standard P2PKH type) with ‘ALL’ flag,
    and completes the 3 missing linking fields in the preimage pushed into unlocking script of maker’s input
    with exactly the same values as in the preimage of his spending input,
    then completes the unlocking script parameters of unlocking script of maker’s input either needed to be
    parsed and used for swapped forward-persistence enforcement or simply part of concatenation for verification of others,
*/
...
function acceptUnsignedSwapOffer (offerTxHex, makerInputSatoshis, makerOutputSatoshis, makerStasInputTxHex, makerStasVout,
  ...
  if (isMakerInputStasScript) {
    ...
    ' ' + preimageTaker +
    ' ' + takerSignatureASM + ' ' + publicKeyTaker.toString('hex'))
  } else if (isP2PKHScript(makerStasInputScript.toHex())) {
    const takerP2pkhSignature = bsv.Transaction.sighash.sign(tx, takerPrivateKey, sighash, 2, makerStasInputScript, new bsv.crypto.BN(makerOutputSatoshis), flags)
