 ...
 */
function acceptSwapOffer (offerTxHex, makerInputSatoshis, makerOutputSatoshis, makerInputTxHex, makerInputVout,
  ...
  // add change
  const extraBytesForPieces = makerInputTxHex.length + takerStasInputTxHex.length
...
function acceptUnsignedSwapOffer (offerTxHex, makerInputTxHex,
  takerPrivateKey, takerInputTxHex, takerStasVout, takerInputSatoshis, takerOutputSatoshis, makerPublicKeyHash, paymentUtxo, paymentPrivateKey) {
  ...
  const takerStasTx = bsv.Transaction(takerInputTxHex)
  const takerOutputScript = takerStasTx.outputs[takerStasVout].script
...
function acceptUnsignedNativeSwapOffer (offerTxHex, takerInputInfo, makerInputTxHex,
  takerPrivateKey, takerInputTxHex, takerStasVout, takerOutputSatoshis, makerPublicKeyHash, paymentUtxo, paymentPrivateKey) {
  console.log('accepting unsigned swap offer')
  // this is the offer tx
  const tx = new bsv.Transaction(offerTxHex)
  const makerInputVout = tx.inputs[0].outputIndex
