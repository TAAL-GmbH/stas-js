...
function acceptSwapOffer (offerTxHex, makerInputTxHex,
  takerPrivateKey, takerInputTxHex, takerInputUTXO, takerOutputSatoshis, makerPublicKeyHash, paymentUtxo, paymentPrivateKey, commissionsInfo) {
//   console.log('accepting swap offer')

  if (commissionsInfo != null && commissionsInfo.length > 0) {
    commissionsInfo.forEach((e) => {
      if(e.address === null || e.satoshis < 1){
        throw new Error('commissionInfo should not have empty address or 0 satoshis')
      }
    })
  }
  ...
  let totalCommissionSats = 0;
  if (commissionsInfo.length > 0) {
    commissionsInfo.forEach(e => {
      commissionPublicKeyHash = addressToPubkeyhash(e.address)
    const commisionLockingScript = bsv.Script.fromASM(`OP_DUP OP_HASH160 ${commissionPublicKeyHash} OP_EQUALVERIFY OP_CHECKSIG`)
    totalCommissionSats += e.satoshis;
    tx.addOutput(new bsv.Transaction.Output({
  ...
  handleChangeForSwap(tx, extraBytesForPieces, paymentPrivateKey.publicKey, totalCommissionSats ? totalCommissionSats : 0)
