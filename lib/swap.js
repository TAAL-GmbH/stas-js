...
function createSwapOffer(makerPrivateKey, makerInputUTXO, wantedInfo) {
  ...
  if (makerPrivateKey === null) {
    throw new KeyError("Maker private key is null");
  }
  if (makerInputUTXO === null) {
    throw new UtxoError("Maker input UTXO is null");
  } else if (
    makerInputUTXO.satoshis < 0 ||
    makerInputUTXO.script === "" ||
    makerInputUTXO.outputIndex < 0 ||
    makerInputUTXO.txId === ""
  ) {
    throw new UtxoError("Invalid maker input UTXO");
  }
  if (typeof makerInputUTXO.script !== "object") {
    throw new UtxoError("makerInputUtxo.script must be an object");
  }

  const makerPublicKeyHash = bsv.crypto.Hash.sha256ripemd160(
    makerPrivateKey.publicKey.toBuffer()
  ).toString("hex");
  const wantedType =
    wantedInfo.type !== undefined && wantedInfo.type === "native"
      ? "native"
      : "token";
  ...
  if (wantedType === "token") {
    ...
    const makerWantedScriptAsm = wantedSlice1
      .concat(makerPublicKeyHash)
      .concat(wantedSlice2);
    const makerWantedScript =
      bsv.Script.fromString(makerWantedScriptAsm).toHex();
    makerWantedLockingScript = bsv.Script.fromHex(makerWantedScript);
  } else {
    makerWantedLockingScript = bsv.Script.fromASM(
      `OP_DUP OP_HASH160 ${makerPublicKeyHash} OP_EQUALVERIFY OP_CHECKSIG`
    );
  }
  ...
  const sighashSingleAnyoneCanPay =
  ...
  if (isMakerOutputStasScript) {
    ...
    const makerSignature = bsv.Transaction.sighash.sign(
      tx,
      makerPrivateKey,
      sighashSingleAnyoneCanPay,
      0,
      makerInputUTXO.script,
      new bsv.crypto.BN(makerInputUTXO.satoshis),
      flags
    );
    const makerSignatureHex = makerSignature.toTxFormat().toString("hex");
    makerUnlockScript = bsv.Script.fromASM(
      makerSignatureHex + " " + makerPrivateKey.publicKey.toString("hex")
    );
  } else {
    ...
    makerUnlockScript = bsv.Script.fromASM(
      makerSignatureASM + " " + makerPrivateKey.publicKey.toString("hex")
    );
  }
...
function acceptSwapOffer(
  ...
  if (offerTxHex === null) {
    throw new SwapError("offerTxHex is null");
  }
  if (makerInputTxHex === null) {
    throw new SwapError("makerInputTxHex is null");
  }
  if (takerPrivateKey === null) {
    throw new SwapError("takerPrivateKey is null");
  }
  if (takerInputTxHex === null) {
    throw new SwapError("takerInputTxHex is null");
  }
  if (takerInputUTXO === null) {
    throw new UtxoError("takerInputUTXO is null");
  }
  if (takerOutputSatoshis === null || takerOutputSatoshis < 1) {
    throw new SwapError("takerOutputSatoshis must be greater than zero");
  }
  if (makerPublicKeyHash === null) {
    throw new SwapError("makerPublicKeyHash is null");
  }
  if (paymentUtxo === null) {
    throw new UtxoError("paymentUtxo is null");
  }
  if (paymentPrivateKey === null) {
    throw new KeyError("paymentPrivateKey is null");
  }

  let hasCommission = false;
  if (
    commissionInfo != null &&
    commissionInfo.address !== null &&
    commissionInfo.satoshis > 0
  ) {
  ...
  const takerPublicKeyHash = bsv.crypto.Hash.sha256ripemd160(
    takerPrivateKey.publicKey.toBuffer()
  ).toString("hex");
  const paymentPublicKeyHash = bsv.crypto.Hash.sha256ripemd160(
    paymentPrivateKey.publicKey.toBuffer()
  ).toString("hex");
  ...
  const takerInputScript =
    takerInputTx.outputs[takerInputUTXO.outputIndex].script;
  ...
  const takerWantedScriptAsm = makerSlice1
    .concat(takerPublicKeyHash)
    .concat(makerSlice2);
  ...
  if (hasCommission) {
    ...
    const commisionLockingScript = bsv.Script.fromASM(
      `OP_DUP OP_HASH160 ${commissionPublicKeyHash} OP_EQUALVERIFY OP_CHECKSIG`
    );
  ...
  handleChangeForSwap(
    tx,
    extraBytesForPieces,
    paymentPrivateKey.publicKey,
    commissionInfo ? commissionInfo.satoshis : 0
  );

  const publicKeyTaker = takerPrivateKey.publicKey;

  const preimageTakerBuf = preimageFn(
    tx,
    sighash,
    1,
    takerInputScript,
    new bsv.crypto.BN(takerInputUTXO.satoshis)
  );
  const preimageTaker = preimageTakerBuf.buf.toString("hex");

  const takerSignature = bsv.Transaction.sighash.sign(
    tx,
    takerPrivateKey,
    sighash,
    1,
  ...
  if (isMakerInputStasScript) {
    const preimageMakerBuf = preimageFn(
      tx,
      sighash,
      0,
      makerInputScript,
      new bsv.crypto.BN(takerOutputSatoshis),
      flags
    );
    ...
    if (hasCommission) {
      ...
      makerUnlockScript = bsv.Script.fromASM(
        numberToLESM(tx.outputs[0].satoshis) +
          ...
          " " +
          commissionPublicKeyHash +
          " " +
          numberToLESM(tx.outputs[3].satoshis) +
          " " +
          paymentPublicKeyHash +
          " " +
          `OP_${paymentUtxo.vout}` +
          " " +
          reversedFundingTXID +
          " " +
  ...
  if (isTakerInputStasScript) {
    ...
    const takerSignatureASM = takerSignature.toTxFormat().toString("hex");
    takerUnlockScript = bsv.Script.fromASM(
      takerSignatureASM + " " + takerPrivateKey.publicKey.toString("hex")
    );
  }

  const paymentSignature = bsv.Transaction.sighash.sign(
    tx,
    paymentPrivateKey,
    sighash,
    2,
    paymentUtxo.scriptPubKey,
    new bsv.crypto.BN(paymentUtxo.satoshis),
    flags
  );
  const paymentSignatureASM = paymentSignature.toTxFormat().toString("hex");
  const paymentUnlockScript = bsv.Script.fromASM(
    paymentSignatureASM + " " + paymentPrivateKey.publicKey.toString("hex")
  );
...
function createUnsignedSwapOffer(makerPrivateKey, makerInputUTXO, wantedInfo) {
  if (makerPrivateKey === null) {
    throw new KeyError("makerPrivateKey is null");
  }

  if (makerInputUTXO === null) {
    throw new UtxoError("makerInputUTXO is null");
  }

  if (wantedInfo === null) {
    throw new SwapError("wantedInfo is null");
  }

  if (wantedInfo.type !== undefined && wantedInfo.type !== "native") {
    throw new SwapError('wantedInfo.type must be undefined or "native"');
  }
...
function acceptUnsignedSwapOffer(
  offerTxHex,
  makerInputTxHex,
  takerPrivateKey,
  takerInputTxHex,
  ...
  if (takerPrivateKey === null) {
    throw new KeyError("takerPrivateKey is null");
  }
  if (takerInputTxHex === null) {
    throw new SwapError("takerInputTxHex is null");
  }
  if (takerInputVout === null) {
    throw new SwapError("takerInputVout is null");
  }
  if (takerInputSatoshis === null || takerInputSatoshis < 1) {
    throw new SwapError("takerInputSatoshis must be greater than zero");
  }
  if (takerOutputSatoshis === null || takerOutputSatoshis < 1) {
    throw new SwapError("takerOutputSatoshis must be greater than zero");
  }
  ...
  const preimageTakerBuf = preimageFn(
    ...
    1,
    takerInputScript,
    new bsv.crypto.BN(takerInputSatoshis)
  );
  const preimageTaker = preimageTakerBuf.buf.toString("hex");
  ...
  if (isTakerInputStasScript) {
    ...
    const takerP2pkhSignature = bsv.Transaction.sighash.sign(
      tx,
      takerPrivateKey,
      sighash,
      1,
      makerInputScript,
      new bsv.crypto.BN(makerOutputSatoshis),
      flags
    );
    const takerP2pkhSignatureASM = takerP2pkhSignature
      .toTxFormat()
      .toString("hex");

    takerUnlockScript = bsv.Script.fromASM(
      takerP2pkhSignatureASM + " " + takerPrivateKey.publicKey.toString("hex")
    );
  }
...
function acceptUnsignedNativeSwapOffer(
  offerTxHex,
  takerInputInfo,
  makerInputTxHex,
  takerPrivateKey,
  ...
  if (takerInputInfo === null) {
    throw new ESwapErrorrror("takerInputInfo is null");
  }
  if (makerInputTxHex === null) {
    throw new SwapError("makerInputTxHex is null");
  }
  ...
  if (isTakerInputStasScript) {
    ...
    const makerSlice1 = makerInputTx.outputs[makerInputVout].script
      .toHex()
      .slice(0, 6);
    const makerSlice2 = makerInputTx.outputs[makerInputVout].script
      .toHex()
      .slice(46);
    takerWantedScript = makerSlice1
      .concat(takerPublicKeyHash)
  ...
  const takerSignature = bsv.Transaction.sighash.sign(
    ...
    sighash,
    1,
    takerInputInfo.utxo.scriptPubKey,
    new bsv.crypto.BN(takerInputInfo.utxo.satoshis),
    flags
  );
  const takerSignatureASM = takerSignature.toTxFormat().toString("hex");
...
function makerSignSwapOffer(
  ...
  if (takerInputTx === null) {
    throw new SwapError("takerInputTx is null");
  }
  if (makerPrivateKey === null) {
    throw new KeyError("makerPrivateKey is null");
  }
  if (takerPublicKeyHash === null) {
    throw new SwapError("takerPublicKeyHash is null");
  }
  if (paymentPublicKeyHash === null) {
    throw new SwapError("paymentPublicKeyHash is null");
  }
  if (paymentUtxo === null) {
    throw new KeyError("paymentUtxo is null");
  }
  ...
  const preimageMakerBuf = preimageFn(
    ...
    0,
    makerOutputScript,
    new bsv.crypto.BN(tx.outputs[1].satoshis)
  );
  const preimageMaker = preimageMakerBuf.buf.toString("hex");
  const makerSignature = bsv.Transaction.sighash.sign(
    tx,
  ...
  if (isMakerOutputStasScript) {
    makerUnlockScript = bsv.Script.fromASM(
      numberToLESM(tx.outputs[0].satoshis) +
        ...
        makerSignatureASM +
        " " +
        makerPrivateKey.publicKey.toString("hex")
    );
  } else {
    const makerSignature = bsv.Transaction.sighash.sign(
      tx,
      ...
      sighash,
      0,
      makerInputTx.outputs[makerInputVout].script,
      new bsv.crypto.BN(tx.outputs[1].satoshis),
      flags
    );
    const makerSignatureASM = makerSignature.toTxFormat().toString("hex");
...
function allInOneSwap(
  ...
  if (takerPrivateKey === null) {
    throw new KeyError("Taker private key is null");
  }
  if (makerInputUtxo === null) {
    throw new UtxoError("Maker input UTXO is null");
  } else if (
  ...
  if (makerInputUtxo.satoshis !== takerOutputSatoshis) {
    throw new UtxoError(
      "makerInputUtxo.satoshis should equal takerOutputSatoshis"
    );
  }
  if (wantedInfo.satoshis !== takerInputSatoshis) {
    throw new SwapError("wantedInfo.satoshis should equal takerInputSatoshis");
  }

  if (makerInputTxHex === null || makerInputTxHex.length < 100) {
    throw new SwapError("Invalid makerInputTxHex");
  }
  if (takerInputTxHex === null || takerInputTxHex.length < 100) {
    throw new SwapError("Invalid takerInputTxHex");
  }
  if (paymentUtxo.txid === null || typeof paymentUtxo.txid.length < 1) {
    throw new UtxoError("paymentUtxo.txid must be a string");
  }
  if (
    paymentUtxo.scriptPubKey === null ||
    typeof paymentUtxo.scriptPubKey !== "string"
  ) {
    throw new UtxoError("paymentUtxo.scriptPubKey must be a string");
  }
  ...
  const makerInputTx = JSON.parse(
    JSON.stringify(bsv.Transaction(makerInputTxHex))
  );
  const makerStasInputScript = bsv.Script.fromHex(
    makerInputTx.outputs[makerInputVout].script
  );
  const makerStasInputScriptASM = makerStasInputScript.toString();
  ...
  const sigSmart = bsv.Transaction.sighash.sign(
    tx,
    makerPrivateKey,
    sighash,
    0,
  ...
  const paymentSignature = bsv.Transaction.sighash.sign(
    ...
    sighash,
    2,
    bsv.Script.fromHex(paymentUtxo.scriptPubKey),
    new bsv.crypto.BN(paymentUtxo.satoshis)
  );
  ...
  const paymentUnlockScript = bsv.Script.fromASM(
    paymentSignatureHex + " " + publicKeyPayment.toString("hex")
  );
...
function handleChangeForSwap(
  ...
  tx.outputs.forEach((output, i) => {
    if (isStasScript(output.script.toHex())) {
      if (tx.inputs[i === 0 ? 1 : 0].output === undefined) {
        ...
        isStasScript(tx.inputs[i === 0 ? 1 : 0].output.script.toHex())
      ) {
        const image = preimageFn(
          tx,
          sighash,
          i,
          tx.inputs[i === 0 ? 1 : 0].output.script,
          tx.inputs[i === 0 ? 1 : 0].output.satoshisBN
        );
        preimageLen += new Varint()
          .fromNumber(image.buf.length)
          .toBuffer().length;
  ...
  let txSizeInBytes =
    tx.toBuffer().length +
    stasInputCount * (9 + 21 + 9 + 21 + 4 + 9 + 33 + 9 + 1 + 72 + 1 + 33) +
    extraDataBytes +
    preimageLen +
    imageBufLength;
  txSizeInBytes +=
    (tx.inputs.length - stasInputCount) * P2PKH_UNLOCKING_SCRIPT_BYTES;
