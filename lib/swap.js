 ...
 */
    function acceptSwapOffer (offerTxHex, makerInputTxHex,
      takerPrivateKey, takerInputTxHex, takerInputUTXO, takerOutputSatoshis, makerPublicKeyHash, paymentUtxo, paymentPrivateKey, extraOutputs) {
    //   console.log('accepting swap offer')
      ...
      if(extraOutputs){
        extraOutputs.forEach(e => {
          tx.addOutput(new bsv.Transaction.Output({
            script: bsv.Script.buildPublicKeyHashOut(e.to).toHex(),
            satoshis: e.amount
          }))
        })
      }
      ...
      console.log('extra outputs ', extraOutputs)
      // add extra outputs if present 
      let addedHexValues = ""
      let changeOutputNum = 0
      if(extraOutputs){ 
        extraOutputs.forEach(e => {
          addedHexValues += ' ' + numberToLESM(e.amount) + ' ' +  bsv.Address.fromString(e.to).toJSON().hash
        })
        changeOutputNum = extraOutputs.length + 2
      }
      ...
      if (isMakerInputStasScript) {
        extraBytesForPieces += takerInputTx.toString().length / 2
      }
      if (isTakerInputStasScript) { 
        extraBytesForPieces += makerInputTx.toString().length / 2
      }
      ...
      if (isMakerInputStasScript) {
        ...
        const makerUnlockScript = bsv.Script.fromASM(
          ...
          addedHexValues +
          ' ' + numberToLESM(tx.outputs[changeOutputNum].satoshis) + ' ' + paymentPublicKeyHash +
...
function acceptUnsignedNativeSwapOffer (offerTxHex, takerInputInfo, makerInputTxHex,
  ...
  const takerSignature = bsv.Transaction.sighash.sign(tx, takerPrivateKey, sighash, 1, takerInputInfo.utxo.scriptPubKey, new bsv.crypto.BN(takerInputInfo.utxo.amount), flags)
  const takerSignatureASM = takerSignature.toTxFormat().toString('hex')
