...
const stasDataRegex = /OP_RETURN [0-9a-fA-F]{40} OP_RETURN (.*)$/
const stasV2DataRegex = /OP_RETURN [0-9a-fA-F]{40} (00|01) (.*)$/
...
function getStasScript (destinationPublicKeyHash, redemptionPublicKey, version, data, isSplittable) {
  const redemptionPublicKeyHash = bsv.crypto.Hash.sha256ripemd160(redemptionPublicKey.toBuffer()).toString('hex')

  switch (version) {
    case 1: {
      let asm = stasV1.replace('[destinationPublicKeyHash]', destinationPublicKeyHash).replace('[redemptionPublicKeyHash]', redemptionPublicKeyHash)
      if (data) {
        asm += ` OP_RETURN ${data}`
      }
...
function getScriptData (script, version) {
  ...
  if (version === 1) {
    const res = b.toString().match(stasDataRegex)
    if (res && res.length > 1) {
      return res[1]
    }
    return null
