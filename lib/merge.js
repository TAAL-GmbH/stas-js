...
function merge (tokenOwnerPrivateKey, contractPublicKey, mergeUtxos, destinationAddr, paymentUtxo, paymentsPrivateKey) {
  ...
  const stasScript = getStasScript(destinationPubkeyHash, contractPublicKey, getVersion(lockingScript), getScriptData(lockingScript))
  ...
  tx.inputs.forEach((input, i) => {
    if (i === 0) {
      ...
      const s = numberToLESM(stasAmount) + ' ' + destinationPubkeyHash +
        ' ' + 'OP_FALSE' + ' ' + 'OP_FALSE' +
