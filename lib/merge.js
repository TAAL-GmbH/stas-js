...
function merge (tokenOwnerPrivateKey, contractPublicKey, mergeUtxos, destinationAddr, paymentUtxo, paymentsPrivateKey) {
  ...
  const symbol = getSymbol(lockingScript)
  const destinationPubkeyHash = addressToPubkeyhash(destinationAddr)
  const stasScript = getStasScript(destinationPubkeyHash, contractPublicKey, version, getScriptData(lockingScript, version), isSplittable(lockingScript), symbol)
