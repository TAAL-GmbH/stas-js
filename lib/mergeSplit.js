...
function mergeSplit (tokenOwnerPrivateKey, contractPublicKey, mergeUtxos, destination1Addr, amountSatoshis1, destination2Addr, amountSatoshis2, paymentUtxo, paymentsPrivateKey) {
  ...
  const stasScript1 = getStasScript(destination1PublicKeyHash, contractPublicKey, version, getScriptData(mergeUtxos[0].scriptPubKey, version), isSplittable(mergeUtxos[0].scriptPubKey))
  ...
  const stasScript2 = getStasScript(destination2PublicKeyHash, contractPublicKey, version, getScriptData(mergeUtxos[0].scriptPubKey, version), isSplittable(mergeUtxos[0].scriptPubKey))
  ...
  tx.inputs.forEach((input, i) => {
    if (i === 0) {
      ...
      const s = numberToLESM(tx.outputs[0].satoshis) + ' ' + destination1PublicKeyHash +
