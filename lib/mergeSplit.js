...
const { KeyError, AddressError, SatAmountError } = require("./errors/error");
...
async function mergeSplit(
  ...
  if (tokenOwnerPrivateKey === null) {
    throw new KeyError("Token owner private key is null");
  }
  ...
  if (amountSatoshis1 === null || amountSatoshis2 === null) {
    throw new SatAmountError("Satoshi value suppled is null");
  }
  const ownerSignatureCallback = async (tx, i, script, satoshis) => {
    return bsv.Transaction.sighash
      .sign(tx, tokenOwnerPrivateKey, sighash, i, script, satoshis)
      .toTxFormat()
      .toString("hex");
  };
  const paymentSignatureCallback = async (tx, i, script, satoshis) => {
    return bsv.Transaction.sighash
      .sign(tx, paymentPrivateKey, sighash, i, script, satoshis)
      .toTxFormat()
      .toString("hex");
  };

  return mergeSplitWithCallback(
    tokenOwnerPrivateKey.publicKey,
    mergeUtxos,
    destination1Addr,
    amountSatoshis1,
    destination2Addr,
    amountSatoshis2,
    paymentUtxo,
    paymentPrivateKey ? paymentPrivateKey.publicKey : null,
