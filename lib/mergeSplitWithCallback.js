...
async function mergeSplitWithCallback(
  ...
  if (destination1Addr === null) {
    throw new AddressError("First destination address is null");
  }
  if (destination2Addr === null) {
    throw new AddressError("Second destination address is null");
  }
  if (
    mergeUtxos === null ||
    ...
    throw new UtxoError("MergeUtxos is invalid");
  }
  if (mergeUtxos.length !== 2) {
    throw new UtxoError("This function can only merge exactly 2 STAS tokens");
  }
  ...
  if (!isMergeScriptSplittable(mergeUtxos)) {
    throw new UtxoError("Cannot Split an NFT");
  }
