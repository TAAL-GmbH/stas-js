...
async function mergeSplitWithCallback (tokenOwnerPublicKey, mergeUtxos, destination1Addr, amountSatoshis1, destination2Addr, amountSatoshis2, paymentUtxo, paymentPublicKey, ownerSignatureCallback, paymentSignatureCallback) {
  ...
  if (issuerPublicKeyHash === destinationPublicKeyHash1 || issuerPublicKeyHash === destinationPublicKeyHash2) {
  ...
  for (let i = 0; i < tx.inputs.length; i++) {
    ...
    if (i === 0) {
      // STAS input
      partialStasMergeSplitScript(mergeUtxos, 1, destination1PublicKeyHash, destination2PublicKeyHash, preimage, paymentPubKeyHash, paymentUtxo, reversedFundingTXID, tx, isZeroFee)
      if (!isUnsigned) {
        const sigASM = await ownerSignatureCallback(tx, i, input.output._script, input.output._satoshisBN)
      ...
      partialStasMergeSplitScript(mergeUtxos, 0, destination1PublicKeyHash, destination2PublicKeyHash, preimageMerge, paymentPubKeyHash, paymentUtxo, reversedFundingTXID, tx, isZeroFee)
      if (!isUnsigned) {
        const sigASM = await ownerSignatureCallback(tx, i, input.output._script, input.output._satoshisBN)
        const endSscript = sigASM + ' ' + tokenOwnerPublicKey.toString('hex')
        const partialScript = tx.inputs[1].script.toASM()
        tx.inputs[1].setScript(bsv.Script.fromASM(partialScript + ' ' + endSscript))
      } else {
        // add to signInfo
        signInfoList.push({
          inputIndex: i,
...
function partialStasMergeSplitScript (mergeUtxos, mergeUtxoIndex, destination1PublicKeyHash, destination2PublicKeyHash, preimage, paymentPubKeyHash, paymentUtxo, reversedFundingTXID, tx, isZeroFee) {
  let s
  if (isZeroFee) {
    s = numberToLESM(tx.outputs[0].satoshis) + ' ' + destination1PublicKeyHash +
      ...
      ' ' + mergeUtxos[mergeUtxoIndex].piece + ' ' + `OP_${mergeUtxos[mergeUtxoIndex].numberOfPieces}` +
      ' ' + preimage
  } else {
    s = numberToLESM(tx.outputs[0].satoshis) + ' ' + destination1PublicKeyHash +
      ' ' + numberToLESM(tx.outputs[1].satoshis) + ' ' + destination2PublicKeyHash +
