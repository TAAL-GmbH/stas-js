...
function issueWithCallback (publicKey, issueInfo, contractUtxo, paymentUtxo, paymentPublicKey, isSplittable, symbol, ownerSignatureCallback, paymentSignatureCallback) {
  ...
  if (!isIssueInfoValid(issueInfo, publicKey)) {
...
function isIssueInfoValid (issueInfo, publicKey) {
  ...
  issueInfo.forEach(info => {
    ...
    console.log(bsv.Address.fromPublicKey(publicKey, process.env.NETWORK).toString())
    if(info.addr === bsv.Address.fromPublicKey(publicKey, process.env.NETWORK).toString()){
      throw new Error('Issuer cannot receieve tokens')
    }
