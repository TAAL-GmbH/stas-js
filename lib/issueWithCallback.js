...
*/
async function issueWithCallback(
  publicKey,
  issueInfo,
  contractUtxo,
  paymentUtxo,
  ...
  if (!isIssueInfoValid(issueInfo)) {
    throw new IssueInfoError("issueInfo is invalid");
  }
  if (!isUtxoValid(contractUtxo)) {
    throw new UtxoError("contractUtxo is invalid");
  }
  if (
    paymentUtxo !== null &&
    (paymentPublicKey === null || paymentSignatureCallback === null)
  ) {
    throw new UtxoError(
  ...
  if (isSplittable === null) {
    throw new IsSplittableError("isSplittable must be a boolean value");
  }
  ...
  if (totalOutSats !== contractUtxo.satoshis) {
    throw new Error(
      `total out amount ${totalOutSats} must equal total in amount ${totalInSats}`
    );
  }
  ...
  issueInfo.forEach((is) => {
    ...
    const stasScript = getStasScript(
      pubKeyHash,
      publicKey,
      data,
      isSplittable,
  ...
  for (let i = 0; i < tx.inputs.length; i++) {
    ...
    if (i === 0) {
      ...
      pubKey = publicKey;
      if (!isUnsigned) {
        sig = await ownerSignatureCallback(
          tx,
          i,
      ...
      if (!isUnsigned) {
        sig = await paymentSignatureCallback(
          tx,
          i,
          input.output._script,
          input.output._satoshisBN
...
function isIssueInfoValid(issueInfo) {
  ...
  issueInfo.forEach((info) => {
    ...
    if (
      info.addr.length < ADDRESS_MIN_LENGTH ||
      info.addr.length > ADDRESS_MAX_LENGTH
    ) {
      throw new Error(
        `issueInfo address must be between ${ADDRESS_MIN_LENGTH} and ${ADDRESS_MAX_LENGTH}`
      );
    }
...
function isUtxoValid(utxo) {
  ...
  if (
    !Object.prototype.hasOwnProperty.call(utxo, "txid") ||
