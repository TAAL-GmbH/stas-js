...
function issue (privateKey, issueInfo, contractUtxo, paymentUtxos, paymentPrivateKey, isSplittable, version) {
  const isZeroFee = (paymentUtxos === null)

  const totalOutSats = issueInfo.reduce((a, b) => a + b.satoshis, 0)
  if (totalOutSats !== Math.round(contractUtxo.amount * 1e8)) {
    console.log(`total out amount ${totalOutSats} must equal total in amount ${Math.round(contractUtxo.amount * 1e8)}`)
    return
  }
