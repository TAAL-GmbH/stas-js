...
async function contractWithCallback(publicKey, inputUtxos, paymentUtxos, paymentPublicKey, schema, tokenSatoshis, ownerSignCallback, paymentSignCallback) {
  ...
  if (typeof schema.totalSupply === 'undefined' || schema.totalSupply === null) {
  ...
  if ((schema.satsPerToken === 'undefined') || schema.satsPerToken === 0 || schema.satsPerToken == null) {
  ...
  for (let inputUtxo in inputUtxos){
    totalSatAmount += inputUtxo.satoshis
  }
  if ((tokenSatoshis * schema.satsPerToken) > totalSatAmount) {
    throw new Error(`Token Supply of ${tokenSatoshis} with satsPerToken of ${schema.satsPerToken} is greater than input amount of ${inputUtxos[0].satoshis}`)
  }

  const issuerPublicKeyHash = bsv.crypto.Hash.sha256ripemd160(publicKey.toBuffer()).toString('hex')
  ...
  if (!isZeroFee) {
    ...
    const issuerPublicKeyHash = bsv.crypto.Hash.sha256ripemd160(publicKey.toBuffer()).toString('hex')
    const changeScript = bsv.Script.fromASM(`OP_DUP OP_HASH160 ${issuerPublicKeyHash} OP_EQUALVERIFY OP_CHECKSIG`)
