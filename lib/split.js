...
function split (tokenOwnerPrivateKey, contractPublicKey, stasUtxo, splitDestinations, paymentUtxos, paymentsPrivateKey) {
  ...
  splitDestinations.forEach(sd => {
    ...
    const stasScript = getStasScript(pkh, contractPublicKey, version, getScriptData(stasUtxo.scriptPubKey))
