...
function split (tokenOwnerPrivateKey, contractPublicKey, stasUtxo, splitDestinations, paymentUtxo, paymentsPrivateKey) {
  ...
  splitDestinations.forEach(sd => {
    ...
    const stasScript = getStasScript(pkh, contractPublicKey, version, getScriptData(stasUtxo.scriptPubKey, version), isSplittable(stasUtxo.scriptPubKey), getSymbol(stasUtxo.scriptPubKey))
