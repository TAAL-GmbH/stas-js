...
function split (tokenOwnerPrivateKey, contractPublicKey, stasUtxo, destination1Addr, amount1, destination2Addr, amount2, paymentUtxos, paymentsPrivateKey) {
  ...
  const stasScript1 = getStasScript(destination1PublicKeyHash, contractPublicKey, getVersion(stasUtxo.scriptPubKey))

  tx.addOutput(new bsv.Transaction.Output({
    script: stasScript1,
    satoshis: Math.floor(amount1 * 1e8)
  }))

  // The second output is the 2nd destination STAS output
  const stasScript2 = getStasScript(destination2PublicKeyHash, contractPublicKey, getVersion(stasUtxo.scriptPubKey))

  tx.addOutput(new bsv.Transaction.Output({
    script: stasScript2,
    satoshis: Math.floor(amount2 * 1e8)
  }))
        ...
        [
          {
            ...
            publicKey: destination1PublicKeyHash
          },
          {
            satoshis: amount2 * 1e8,
            publicKey: destination2PublicKeyHash
          },
          {
