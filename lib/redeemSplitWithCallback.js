...
*/
async function redeemSplitWithCallback(
  tokenOwnerPublicKey,
  contractPublicKey,
  stasUtxo,
  splitDestinations,
  ...
  if (contractPublicKey === null) {
    throw new KeyError("contract public key is null");
  }
  if (tokenOwnerPublicKey === null) {
    throw new KeyError("token owner public key is null");
  }
  if (splitDestinations === null || splitDestinations.length === 0) {
    throw new SplitDestionError("split destinations array is null or empty");
  }

  if (stasUtxo === null) {
    throw new UtxoError("stasUtxo is null");
  }

  if (paymentUtxo !== null && paymentPublicKey === null) {
    throw new UtxoError("Payment UTXO provided but payment public key is null");
  }
  ...
  if (!isSplitScriptSplitable(stasUtxo)) {
    throw new IsSplittableError("Cannot Split an NFT");
  }
  ...
  if (redeemSats <= 0) {
    throw new SatAmountError(
      "Not enough input Satoshis to cover output. Trying to redeem " +
        redeemSats +
        " sats"
    );
  }
  ...
  const publicKeyHash = bsv.crypto.Hash.sha256ripemd160(
    contractPublicKey.toBuffer()
  ).toString("hex");

  const redeemScript = bsv.Script.fromASM(
  ...
  segments.push({
    satoshis: redeemSats,
    publicKey: bsv.crypto.Hash.sha256ripemd160(
      contractPublicKey.toBuffer()
    ).toString("hex"),
  });
  ...
  for (let i = 0; i < tx.inputs.length; i++) {
    ...
    if (i === 0) {
      // STAS input
      partialSTASUnlockingScript(tx, segments, version, isZeroFee);
      if (!isUnsigned) {
        const signature = await ownerSignatureCallback(
          tx,
          i,
        ...
        const endSscript =
          signature + " " + tokenOwnerPublicKey.toString("hex");
