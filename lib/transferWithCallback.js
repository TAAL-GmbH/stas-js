...
const { KeyError, UtxoError, AddressError } = require("./errors/error");
...
*/
async function transferWithCallback(
  tokenOwnerPublicKey,
  stasUtxo,
  destinationAddress,
  paymentUtxo,
  ...
  if (paymentUtxo !== null && paymentPublicKey === null) {
    throw new UtxoError("Payment UTXO provided but payment key is null");
  }
  if (paymentUtxo === null && paymentPublicKey !== null) {
    throw new UtxoError("Payment public key provided but payment UTXO is null");
  }
  ...
  if (destinationAddress === null) {
    throw new AddressError("destination address is null");
  }

  try {
    bsv.Address.fromString(destinationAddress);
  } catch (e) {
    throw new AddressError("Invalid destination address");
  }
  ...
  for (let i = 0; i < tx.inputs.length; i++) {
    ...
    if (i === 0) {
      ...
      if (!isUnsigned) {
        signature = await ownerSignatureCallback(
          tx,
          i,
