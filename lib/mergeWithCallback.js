...
async function mergeWithCallback (tokenOwnerPublicKey, mergeUtxos, destinationAddr, paymentUtxo, paymentPublicKey, ownerSignatureCallback, paymentSignatureCallback) {
  ...
  for (let i = 0; i < tx.inputs.length; i++) {
    ...
    if (i === 0) {
      // STAS input
      partialStasMergeScript(mergeUtxos, 1, destinationPubkeyHash, stasAmount, preimage, paymentPubKeyHash, paymentUtxo, reversedFundingTXID, tx, isZeroFee)
      ...
      partialStasMergeScript(mergeUtxos, 0, destinationPubkeyHash, stasAmount, preimageMerge, paymentPubKeyHash, paymentUtxo, reversedFundingTXID, tx, isZeroFee)
...
function partialStasMergeScript (mergeUtxos, mergeUtxoIndex, destinationPubkeyHash, stasAmount, preimageMerge, paymentPubKeyHash, paymentUtxo, reversedFundingTXID, tx, isZeroFee) {
  let s
  if (isZeroFee) {
